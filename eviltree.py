...
parser.add_argument("-x", "--regex", action="store", help = "Regex pattern to search for in files.")
...
parser.add_argument("-c", "--case-sensitive", action="store_true", help = "Enables case sensitive keyword search.")
...
default_keywords = ['passw', 'admin', 'token', 'user', 'secret', 'login', 'db_user', 'db_passw']
keywords = []
...
def file_inspector(file_path, mode = 0):
	...
	if args.regex:
		return MATCH if not args.verbose else [MATCH, f" {GREEN}[{', '.join(matched)}]{END}"]
